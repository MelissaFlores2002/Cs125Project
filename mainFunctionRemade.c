...
int main() {
  ...
  for (loopCounter=0; loopCounter<6; loopCounter++){
    ...
    printf("\ninnocentsArray[%d] = %d\n", loopCounter, innocentsArray[loopCounter]);
  }
  //making it so that no numbers are repeated
  for (loopCounter=0; loopCounter<6;loopCounter++){
    for (secondLoopCounter=0; secondLoopCounter<6; secondLoopCounter++){
      if (secondLoopCounter==loopCounter){
        continue;
      }
      while (innocentsArray[loopCounter] == innocentsArray[secondLoopCounter]){
        innocentsArray[secondLoopCounter]=randomNumber(7);
      }
    }
  printf("\ninnocentsArray[%d] = %d\n", loopCounter, innocentsArray[loopCounter]);
  }
  ...
  while ((inputCheck !=1) || ((userInput != weap1) && (userInput != weap2) && (userInput != weap3))){
    printf("Bad input, try again.\nWhich one would you like to INVESTIGATE first? ");
    inputCheck=scanf("%d", &userInput);
  }
  ...
  while (userInput == 1) {
    //prompting the user to INVESTIGATE the weapons
    printf("\nWhich one would you like to INVESTIGATE next? (please type an integer): ");
    inputCheck=scanf("%d", &userInput);
    //error checking
    while ((inputCheck !=1) || ((userInput != weap1) && (userInput != weap2) && (userInput != weap3))){
      printf("Bad input, try again.\nWhich one would you like to INVESTIGATE first? ");
    ...
    //testing to see if their input is the correct weapon
    testWeap(userInput, detectiveWeaponOne, detectiveWeaponTwo, weapon);
    printf("\nWould you like to keep invesitgating? (1 for yes, 2 for no): ");
    scanf("%d", &userInput);
  ...
  printf("\nCHOOSE WHO YOU'D LIKE TO INVESTIGATE (please enter a number): ");
  scanf("%d", &userInput);
  //Error checking
  errorCheckTest = errorCheck(userInput, 7, "\nCHOOSE WHO YOU'D LIKE TO INVESTIGATE (please enter a number): ");
  userInput = errorCheckTest.userInput;
